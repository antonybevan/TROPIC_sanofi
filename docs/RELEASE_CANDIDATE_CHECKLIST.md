...
Generated: 2026-08-24 18:28:17 UTC
...
| --- | --- | --- |
| G04 analysis_dataset_promotion | Current live run uses real independent SAS engine | Run `python3 platform/cibuild.py --real-sas` with ODA/local SAS and bind outputs to that run. |
| G04 analysis_dataset_promotion | Dataset reconciliation is non-simulated | Re-run reconciliation from a real SAS production track; simulated zero-diff is tautological. |
| G06 qc_signoff | F-042 pain-response subject-level SAS/R reconciliation passes | Run the real-SAS F-042 response extract and reconcile every subject/event date, confirmation date, component, and date source against the R derivation. |
...
| G06 qc_signoff | Simulation methods evidence is precise, verified, and remains non-qualified | PASS | platform/simulation_operating_characteristics/simulation_oc_status.json execution=PASS; precision=PASS; design=PASS; qualification=NOT_QUALIFIED |  |
...
| G04 analysis_dataset_promotion | Current live run uses real independent SAS engine | BLOCKER | platform/pipeline_health.json sas_execution_mode=sim; pipeline=RED | Run `python3 platform/cibuild.py --real-sas` with ODA/local SAS and bind outputs to that run. |
| G04 analysis_dataset_promotion | Dataset reconciliation is non-simulated | BLOCKER | platform/reconciliation_status.json overall=FAIL; simulated=False | Re-run reconciliation from a real SAS production track; simulated zero-diff is tautological. |
| G06 qc_signoff | F-042 pain-response subject-level SAS/R reconciliation passes | BLOCKER | platform/reconciliation_status.json endpoint_controls.F042_PAIN_RESPONSE=FAIL | Run the real-SAS F-042 response extract and reconcile every subject/event date, confirmation date, component, and date source against the R derivation. |
...
| G04 analysis_dataset_promotion | Third-engine admiral reconciliation passes for critical core on current run | PASS | platform/admiral_reconciliation_status.json overall=PASS; dag_stage=missing; current_with_run=True |  |
...
| G09 release_candidate_lock | Current release-run manifest is release-candidate grade (full DAG, clean tree) | BLOCKER | platform/release_run_manifest/release_run_manifest.json status=FAIL; evidence_grade=failed_binding; run_scope=partial_dag; seal=5cfe39b98f1a440fcb977e7b179e284157800eaa1280d3c80f584e2c4d8c6950 | Release manifest must be status=PASS (not REMEDIATION/FAIL): full current DAG, clean worktree, and current-run binding. REMEDIATION is valid development evidence only. |
| G09 release_candidate_lock | Pipeline health records a full current DAG run | BLOCKER | platform/pipeline_health.json run_scope=partial_dag; stages_recorded=18; stages_expected=41; stages_not_run=23 | Run the full DAG from stage 1 (no --from-stage) under --real-sas so pipeline_health covers every study_manifest stage. |
