# F-042 PR Source Qualification Audit

**Audit ID:** `F042-PR-SOURCE-QUAL-2026-08-03`<br>
**Status:** **PRE-IMPLEMENTATION PROFILE COMPLETE — NOT APPROVED FOR ENDPOINT USE**<br>
...
This audit establishes whether the raw SDTM PR domain is technically available and linkable for a future approved palliative-radiotherapy rule. It is a source-quality and staging-readiness assessment only.
...
The date-only linkage does not answer whether a later procedure should alter an earlier pain event. The approved SAP window, treatment intent, supporting-disease evidence, and event precedence remain open under ED-01 through ED-03.
...
| Preserve source text | `PRTRT`, `PRCAT`, `PRDTC`, `VISIT`, and keys are present | **Required** — no recoding of treatment intent at ingest. |
...
## 5. Draft staging specification — pending ED-02
...
6. Apply the approved source precedence and treatment-intent rule only in a downstream endpoint derivation layer.
7. Add SAS/R regression fixtures for duplicate keys, partial dates, same-day records, post-randomization timing, and records with non-radiation procedure terms.
...
Before PR is consumed by an endpoint program, the parent decision record must contain:

- approved primary source and CM precedence;
