...
The 41-stage pipeline (`platform/cibuild.py`, gated by `.github/workflows/ci.yml`)
...
**Wiring status.** The modern exports (`export_datasetjson.py`, `build_ars.py`,
`build_usdm.py`) and the eCTD sequence controls (`build_ectd_backbone.py`,
`materialize_ectd.py`) are named manifest stages gated by their own exit codes in
the full pipeline. They remain documented here for
...
| 3 | `platform/export_datasetjson.py` | `04_analysis_datasets/adam/*_prod.xpt`, SDTM `*.xpt` | `04_analysis_datasets/datasetjson/**/*.json` (Dataset-JSON v1.1) | yes | **Dataset-JSON Export (v1.1)** |
| 4 | `platform/build_ars.py` | MP-arm KM results | `05_outputs/ars/` (ARS v1.0 ReportingEvent + ARD) | yes | **Analysis Results Standard (ARS v1.0)** |
| 5 | `platform/build_usdm.py` | study metadata / `config/study_config.yaml` | `03_metadata/usdm/` (USDM v3.0 study definition) | no (study-def only) | **USDM Study Definition (v3.0) + CI** |
| 6 | `platform/build_ectd_backbone.py` | `08_submission_package/m5/` tree | `08_submission_package/ectd/0000/` backbone + STF + `index-md5.txt` | yes (checksums package leaves) | **eCTD Backbone + STF (sequence 0000)** |
| 7 | `platform/materialize_ectd.py` | `08_submission_package/ectd/0000/` backbone + `08_submission_package/m5/` | content copied into `08_submission_package/ectd/0000/`, MD5 re-verified | yes | **Materialize eCTD Sequence** |
...
Each step is deterministic for fixed inputs. The modern exports are additive; the uplift
and sequence steps deliberately refresh controlled derived/package outputs but never modify
the pristine source. Committed artifacts are the data-free outputs and the
