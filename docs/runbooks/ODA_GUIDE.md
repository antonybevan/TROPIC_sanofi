...
```bash
python3 platform/seed_sdtm.py            # uploads only if the ODA manifest mismatches local
python3 platform/seed_sdtm.py --force    # override: re-upload regardless
```
...
```
The manifest-named `SAS Production (ODA/Real/Simulated)` stage:
...
3. Runs `00_master_driver.sas`, captures the log to `04_analysis_datasets/programs/sas/oda_master_driver.log`,
...
`platform/pipeline_health.json` records exactly what happened:
