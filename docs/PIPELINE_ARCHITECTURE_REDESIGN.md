...
## 10. Implementation disposition (2026-08-23)

> The delivery dashboard, domain control reports, orchestrator integration, and CI
> checks described below have been implemented. This section now records the design
> decision rather than an unfinished next step. The physical code/evidence directory
> split remains deferred; the tiered platform index and dual reviewer/engineer surfaces
> provide the intended separation without destabilizing controlled paths.

The no-move architecture report is generated locally as
`docs/DELIVERY_EVIDENCE_DASHBOARD.md` by
[`platform/build_delivery_dashboard.py`](../platform/build_delivery_dashboard.py) from `config/evidence_layers.yaml` and
`config/delivery_workstreams.yaml`. Domain-specific control reports now use this
model. The implementation disposition is:

```text
1. CI integration — implemented for the complete data-free test/control surface.
2. Physical code/evidence split — intentionally deferred as a high-churn refactor.
3. Orchestrator integration of architecture reports — implemented through the
   manifest-driven controls and delivery-control builder.
```

Future structural moves require a separate migration plan and must preserve the
manifest, package paths, clean-clone tests, and release hash bindings.
