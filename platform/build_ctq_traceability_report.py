...
TFL_INDEX = "platform/tfl_output_index/tfl_output_index.csv"
ARS_ARD = "05_outputs/ars/tropic_ard.csv"
VALIDATION_STRATEGY = "config/validation_strategy.yaml"
...
def build_ctq_report(root, register_path, out_dir, report_path):
    ...
    for item in register.get("ctq_factors", []):
        ...
        for validation_id in item.get("validation_artifacts", []):
            if validation_id not in validation_ids:
                failures.append(f"validation artifact {validation_id} missing from config/validation_strategy.yaml")
    ...
    lines = [
        ...
        "",
        "> This report evaluates `config/ctq_traceability.yaml`, linking clinical questions "
...
def main(argv=None):
    ...
    parser.add_argument("--register", default="config/ctq_traceability.yaml")
    parser.add_argument("--out-dir", default="platform/ctq_traceability")
