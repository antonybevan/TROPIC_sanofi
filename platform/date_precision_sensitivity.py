...
Mitoxantrone arm (TRT01P="MP"; the synthetic CbzP arm is excluded and enforced out of
the analysis set).
...
percentiles, max absolute shift). A perturbation band no wider than one month, with a
maximum median shift no wider than one month, is treated as robust to the source date
precision.

Assumptions (stated for the reviewer): jitter is applied to the net analysis time
`AVAL`, modeling the week-offset uncertainty in the event/censor date while treating the
origin date as day-precision. If both origin and event dates were independently
week-binned, this +/-3.5d net-AVAL model should be read as a lower-bound sensitivity
rather than a conservative combined-date model. The +/-3.5d window does not flip event
vs censor status (true for sub-week perturbations).

USAGE:  python3 platform/date_precision_sensitivity.py
...
from tte_utils import ARM_VALUE, ARM_VAR, km_median_days, tte_analysis_set

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
ADTTE = os.path.join(ROOT, "04_analysis_datasets/adam", "adtte_prod.xpt")
OUT_JSON = os.path.join(ROOT, "platform", "conformance",
...
def main():
    ...
    hdr = (f"{'PARAM':7} {'N':>4} {'Evt':>4} {'pt med (d)':>11} {'pt med (mo)':>11} "
           f"{'MC 2.5%':>9} {'MC 97.5%':>9} {'max|shift|d':>11} {'verdict':>12}")
    print(hdr)
    print("-" * len(hdr))

    for p in params:
        sub, counts = tte_analysis_set(
            df,
            p,
            [{"variable": ARM_VAR, "comparator": "EQ", "value": [ARM_VALUE]}],
        )
        aval = sub["AVAL"].to_numpy(dtype=float)
        event = (sub["CNSR"].to_numpy(dtype=float) == 0).astype(int)
        n = len(sub)
        nev = int(event.sum())
        pt = km_median_days(aval, event)

        meds = np.empty(N_REP)
        for i in range(N_REP):
            jit = rng.uniform(-JITTER, JITTER, size=n)
            perturbed = np.maximum(aval + jit, 1.0)
            meds[i] = km_median_days(perturbed, event)
        ...
        if len(finite) == 0:
            ...
            if np.isfinite(pt):
                ...
                # KM median not reached (<50% events) at baseline and in every replicate:
                # source date precision cannot change a not-reached median.
                verdict = "ROBUST (n/r)"
        else:
        ...
        results[p] = {
            "n": n, "source_n": counts.source_n,
            "excluded_missing_aval_cnsr": counts.excluded_missing,
            "excluded_non_mp": counts.excluded_nonmatching,
            "arm_variable": ARM_VAR,
            "arm_value": ARM_VALUE,
            "events": nev, "median_reached_fraction": round(reached_frac, 3),
    ...
    with open(OUT_JSON, "w", encoding="utf-8") as fh:
        json.dump({"jitter_days": JITTER, "replicates": N_REP, "seed": SEED,
                   "arm": f"{ARM_VALUE} (real) only",
                   "arm_variable": ARM_VAR,
                   "parameters": results}, fh, indent=2)
    print(f"\nWrote {os.path.relpath(OUT_JSON, ROOT)}")
    overall = "ROBUST" if all(v["verdict"].startswith("ROBUST")
                              for v in results.values()) else "MIXED"
