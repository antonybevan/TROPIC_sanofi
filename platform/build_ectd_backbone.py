...
WHY THIS EXISTS
---------------
`08_submission_package/m5/` holds the correct FDA Study-Data folder tree and both Define-XML files, but it
...
missing eCTD backbone layer, wiring the existing Module 5 deliverables with real MD5
...
-----------------------------------------------------
  08_submission_package/ectd/0000/index.xml                       eCTD backbone
...
def classify(rel: str):
    ...
    if base == "blankcrf.pdf":
        # Source (blank) CRF — NOT an annotated CRF (controlled scope: no aCRF claim,
...
def collect():
    """Walk 08_submission_package/m5/ and return ordered content items, excluding only file types that are not
