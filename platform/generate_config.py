...
`config/study_config.yaml` is source-controlled study input, but it is still text that becomes
...
class ConfigError(ValueError):
    """Raised when config/study_config.yaml cannot be converted into safe SAS macro source."""


def parse_yaml(filepath):
    if yaml is None:
        raise ConfigError("PyYAML is not importable; cannot read config/study_config.yaml")
    try:
...
def main():
    ...
    proj_root = os.getcwd()
    if not os.path.exists(os.path.join(proj_root, "config/study_config.yaml")):
        proj_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    yaml_path = os.path.join(proj_root, "config/study_config.yaml")
    sas_out_path = os.path.join(proj_root, "04_analysis_datasets/programs/sas", "00_config_generated.sas")
