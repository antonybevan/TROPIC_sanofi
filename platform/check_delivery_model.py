...
def check_delivery_model(model_path, evidence_path):
    root = os.path.abspath(os.path.join(os.path.dirname(model_path) or ".", ".."))
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Validate config/delivery_workstreams.yaml")
    parser.add_argument("--model", default="config/delivery_workstreams.yaml")
