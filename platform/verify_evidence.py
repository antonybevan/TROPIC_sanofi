...
platform/pipeline_health.json and reconciliation_status.json are LIVE telemetry, overwritten
by every run -- including a sim/--demo run -- so they can never serve as durable proof (audit
C-1). The platform/evidence/ snapshot is the durable artifact instead, but a committed
...
coverage of the config/study_manifest.yaml dataset set are all that is staticly checkable without a
...
actual XPTs are present locally (a data-bearing checkout, or 04_analysis_datasets/adam/ left over from a prior run),
...
EVIDENCE_DIR = os.path.join(ROOT, "platform", "evidence")
_ROW_RE = re.compile(
    ...
    try:
        ...
        if set(rows) != expected:
            problems.append(f"evidence md5 manifest datasets {sorted(rows)} do not match "
                            f"config/study_manifest.yaml datasets {sorted(expected)}.")
    except manifest.ManifestError as e:
        problems.append(f"config/study_manifest.yaml could not be loaded to verify evidence coverage: {e}")
...
def check_hot(evidence_dir=EVIDENCE_DIR, adam_dir=os.path.join(ROOT, "04_analysis_datasets/adam")):
    ...
    *_prod.xpt/*_v.xpt are actually present locally (a data-bearing checkout or leftover
    04_analysis_datasets/adam/ from a prior run). Recomputes md5 and reports whether it matches the committed
