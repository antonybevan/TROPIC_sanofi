...
for (ds in ds_names) {
  f <- sprintf("04_analysis_datasets/adam/%s_prod.xpt", tolower(ds))
...
write.csv(res, "06_qc_evidence/conformance/adam_conformance_report.csv", row.names = FALSE)
...
jsonlite::write_json(list(status = status, errors = n_err_real, warnings = n_warn,
                     ...
                     "06_qc_evidence/conformance/adam_conformance_status.json", auto_unbox = TRUE)
...
md <- c(
  ...
  sprintf("*Engine:* in-repo `adam_conf_check.R` — interim gate (NOT the full FDA Validator pack; see `06_qc_evidence/conformance/p21_adam_runrecord.md`).  "),
  sprintf("*Inputs:* 7 ADaM `*_prod.xpt` + `03_metadata/define/define.xml`.  "),
