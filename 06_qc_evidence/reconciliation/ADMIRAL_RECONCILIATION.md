...
(`04_analysis_datasets/programs/sas/`) and hand-rolled R validation (`04_analysis_datasets/programs/r/v_*`) tracks.
...
| File | Role |
|---|---|
| `04_analysis_datasets/programs/r/admiral_adsl.R` | admiral ADSL core (`derive_vars_merged`, `derive_var_trtdurd`) → `04_analysis_datasets/adam/adsl_admiral.xpt` |
| `04_analysis_datasets/programs/r/admiral_adtte.R` | admiral ADTTE OS + PFS (`derive_param_tte`, `event_source`/`censor_source`) → `04_analysis_datasets/adam/adtte_admiral.xpt` |
| `06_qc_evidence/reconciliation/admiral_reconcile.R` | scoped, exact (0-tolerance) diff vs `*_prod.xpt` → `platform/admiral_reconciliation_status.json` |

Run order:
```bash
Rscript 04_analysis_datasets/programs/r/admiral_adsl.R
Rscript 04_analysis_datasets/programs/r/admiral_adtte.R
Rscript 06_qc_evidence/reconciliation/admiral_reconcile.R
```
...
As of the controlled-DAG wiring, this track is **in** `config/study_manifest.yaml`
`infrastructure_stages.post` (after Cross-Language Audit Reconcile):

1. `Admiral ADSL Re-derivation` → `04_analysis_datasets/programs/r/admiral_adsl.R`
2. `Admiral ADTTE Re-derivation (OS/PFS)` → `04_analysis_datasets/programs/r/admiral_adtte.R`
3. `Admiral Core Reconciliation` → `06_qc_evidence/reconciliation/admiral_reconcile.R` (**gated**)

`cibuild.py` fails the build if `admiral_reconciliation_status.json` is not
`overall: PASS`. `config/validation_strategy.yaml` further requires the status file to be
