...
def classify(rel: str, suffix: str) -> tuple[str, str]:
    ...
    if low.startswith("04_analysis_datasets/programs/r/") or low.startswith("06_qc_evidence/reconciliation/") or top == "tests":
        return "validation/QC", "Independent R derivation, reconciliation, validation, or regression-test artifact."
    if low.startswith("04_analysis_datasets/adam/"):
    ...
    if low.startswith("07_reviewer_explanation/guides/") or top == "docs" or suffix == ".md" or suffix == ".docx":
        return "documentation", "Study documentation, reviewer guide, analysis plan, reproducibility record, or project narrative."
    if low.startswith("05_outputs/tfl/"):
    ...
    if low.startswith("04_analysis_datasets/datasetjson/") or top == "08_submission_package":
    ...
    if low.startswith("05_outputs/ars/") or low.startswith("03_metadata/usdm/"):
        if suffix in {".py", ".r", ".sh"}:
    ...
    if top.startswith(".") or rel in {"config/study_config.yaml", "config/study_manifest.yaml", "sascfg_personal.py", "_authinfo"}:
