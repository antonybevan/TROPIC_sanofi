...
for name, keys in KEYS.items():
    prod, _ = pyreadstat.read_xport(str(ROOT / f"04_analysis_datasets/adam/{name}_prod.xpt"), disable_datetime_conversion=True)
    val, _ = pyreadstat.read_xport(str(ROOT / f"04_analysis_datasets/adam/{name}_v.xpt"), disable_datetime_conversion=True)
...
out = ROOT / "06_qc_evidence/audit/dual_language_comparison.csv"
