...
| ADaM define declares the datasets that exist | `IG.*` in `define.xml` vs `04_analysis_datasets/adam/*_prod.xpt` | 7 ADaM match; `clinsite` correctly BIMO-only (BDRG), not in ADaM define |
...
(`platform/date_precision_sensitivity.py`, 2,000 MC replicates, ±3.5-day jitter on
...
- **m-4 — eCTD packaging.** Backbone/STF were absent; now scaffolded in `08_submission_package/ectd/0000/`
...
`results_reconciliation_status.json`, `03_metadata/define/define{,_sdtm}.xml`,
`04_analysis_datasets/adam/*_prod.xpt`, `05_outputs/tfl/.../T-11-Efficacy_Tables.txt`. Reviewer inputs:
`07_reviewer_explanation/guides/{ADRG,SDRG,BDRG,TRACEABILITY_MATRIX}.md`.*
...
|---|---|---|
| **T-1** SDTMIG 3.1.1 → **3.4** | Derived 3.4 uplift (`platform/uplift_sdtm_34.R`): AGE from de-id AGEGRP, ACTARM/ACTARMCD, AESOC, EPOCH (VISIT-based), EXENDY, week-vars→SUPPAE/SUPPDS, TA; define regenerated (`uplift_define_34.py`). Source stays pristine. | XSD-VALID; 315 ref checks PASS |
...
| **m-4** eCTD packaging | Backbone + STF + **content materialized** (89/89 leaves MD5-verified) and **all 3 XML files DTD-VALID** (ICH eCTD 3.2 / ICH STF 2.2 / FDA Regional v3.3). | `08_submission_package/ectd/RUN_RECORD.md` |
