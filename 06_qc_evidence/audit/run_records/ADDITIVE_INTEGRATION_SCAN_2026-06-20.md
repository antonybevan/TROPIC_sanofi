...
- **TROPIC today:** has ARM v1.0 (8 ResultDisplays / 10 AnalysisResults in `define.xml`)
  and the numeric results already live in `05_outputs/tfl/output/tables/*` — but **no ARS
...
- **TROPIC today:** has a protocol PDF, SAP `.docx`, and `config/study_config.yaml`/
  `config/study_manifest.yaml` — structured inputs — but **no USDM instance.**
- **Build here:** assemble a USDM v3 `Study` JSON from `config/study_config.yaml` + protocol
...
### F. eCTD v4.0 (ICH RPS)
- I generated a v3.2.2 backbone (`08_submission_package/ectd/0000/`). v4.0/RPS is the forward target but is
...
|---|---|---|---|---|
| A | ARS v1.0 + ARD | High | Yes | **BUILT** — `05_outputs/ars/` (validation PASS) |
| B | USDM / DDF study def | High | Yes | **BUILT** — `03_metadata/usdm/` (usdm_model-validated) |
| C | SDSP document | Med | Yes | **BUILT** — `07_reviewer_explanation/guides/SDSP.md` |
| D | Dataset-JSON NDJSON | Med | Yes | **BUILT** — `04_analysis_datasets/datasetjson/**/*.ndjson` (lossless) |
...
for all 42 datasets (round-trip lossless). Generators: `platform/build_ars.py`,
