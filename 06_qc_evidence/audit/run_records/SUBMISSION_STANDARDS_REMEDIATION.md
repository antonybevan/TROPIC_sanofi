...
(`04_analysis_datasets/programs/sas/U_xpt_export.sas`, "Standard: CDISC compliant transport v5
...
- New exporter `platform/export_datasetjson.py` (XPT → Dataset-JSON v1.1).
- New outputs `04_analysis_datasets/datasetjson/adam/*.json` (8) + `04_analysis_datasets/datasetjson/sdtm/*.json` (34).
- New `04_analysis_datasets/datasetjson/README.md`.
...
**Evidence:** `03_metadata/define/define_sdtm.xml` →
`def:Standard … Name="SDTMIG" Version="3.1.1"`; `07_reviewer_explanation/guides/SDRG.md` →
"**Standard:** CDISC SDTMIG v3.1.1"; `04_analysis_datasets/programs/sas/S_sdtm_mapping.sas` builds
"under the trial-era SDTM-IG 3.1.1 standard". The project's own
`platform/conformance/CORE_RUN_RECORD.md` records that CORE's **lowest**
...
**Fix (additive):** `platform/build_ectd_backbone.py` generates eCTD sequence
0000 under `08_submission_package/ectd/0000/` — `index.xml` (87 content leaves), `stf-tropic.xml`
...
See `08_submission_package/ectd/RUN_RECORD.md`.

**Finalized (2026-06-20):** (a) **DONE** — `platform/materialize_ectd.py` copies the
`m5/` content under `08_submission_package/ectd/0000/` per the index manifest and re-verifies every leaf
...
**Was:** the 42 generated Dataset-JSON files (`04_analysis_datasets/datasetjson/`) are patient-level
...
**Fix:** added `04_analysis_datasets/datasetjson/**/*.json` to `.gitignore` (README/docs still
tracked) and a guard for `08_submission_package/ectd/**/*.{xpt,sas7bdat,json}`. Verified: the data JSON
is now ignored, `04_analysis_datasets/datasetjson/README.md` stays tracked.

## Files added across this remediation (additive only — nothing existing changed)

- `platform/export_datasetjson.py` — XPT → Dataset-JSON v1.1 exporter
- `04_analysis_datasets/datasetjson/adam/*.json` (8), `04_analysis_datasets/datasetjson/sdtm/*.json` (34), `04_analysis_datasets/datasetjson/README.md`
- `platform/build_ectd_backbone.py` — eCTD backbone + STF generator
- `08_submission_package/ectd/0000/…` — `index.xml`, `index-md5.txt`, `us-regional.xml`, `stf-tropic.xml`, util DTD readme
- `08_submission_package/ectd/RUN_RECORD.md`
- `.gitignore` — data-leak hardening for the new exports
- `06_qc_evidence/audit/run_records/SUBMISSION_STANDARDS_REMEDIATION.md` (this file)
...
(`01_source_data/real_sdtm/`, SDTMIG 3.1.1) is **unmodified**; a derived 3.4 layer is
produced deterministically and packaged.

| Item | What was done | Proof |
|---|---|---|
| **1 — SDTMIG 3.4** | `platform/uplift_sdtm_34.R` derives the 3.4 SDTM (AGE from de-id AGEGRP, ACTARM/ACTARMCD, AESOC, EPOCH from VISIT, EXENDY, week-vars→SUPPAE/SUPPDS, TA, drop redundant SUBJID, library var order). `define_sdtm.xml` regenerated to 3.4 via `03_metadata/define/uplift_define_34.py`. | XSD-VALID (Define 2.1); 315 ref-integrity checks PASS |
| **2 — CORE report (SDTM)** | Real CORE 0.16.0 run at `-s sdtmig -v 3.4`. All targeted structural rules cleared; 0 structural-fixable residual. | `platform/conformance/{core_sdtm34_report.json, CORE_SDTM34_RUN_RECORD.md}` |
...
**New files:** `platform/uplift_sdtm_34.R`, `03_metadata/define/uplift_define_34.py`,
