...
The frozen pre-audit snapshot excludes only the repository's administrative root `.git/` and the auditor-created `06_qc_evidence/audit/` directory. The nested CORE-engine checkout, its `.git`, all environments, ignored local files, runtime caches, datasets and outputs are included.
...
flowchart LR
  ...
  MAT --> SEQ["08_submission_package/ectd/0000"]
...
- 44 individually named unindexed XPT files under `08_submission_package/ectd/0000/m5/` (eight ADaM/BIMO and 35 SDTM copies, with one BIMO duplicate path).
...
- [x] Frozen snapshot enumerated recursively, excluding only root `.git/` administration and auditor-created `06_qc_evidence/audit/`.
