...
OUT = ROOT / "06_qc_evidence/audit/dataset_metadata.csv"


def paths():
    for dirpath, dirnames, filenames in os.walk(ROOT):
        directory = Path(dirpath)
        dirnames[:] = [d for d in dirnames if not (directory == ROOT and d in {".git"})]
...
for path in sorted(paths()):
    ...
    try:
        ...
        if rel.startswith(("01_source_data/", "04_analysis_datasets/adam/", "08_submission_package/", ".core_run/sdtm34/")):
