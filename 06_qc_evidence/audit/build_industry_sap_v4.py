...
OUT = ROOT / "02_specifications/sap/TROPIC_SAP_v4.0_industry_grade.docx"
...
def rows_from_workbook(sheet: str) -> list[dict[str, str]]:
    wb = load_workbook(ROOT / "03_metadata/adam" / "ADaM_spec.xlsx", data_only=True, read_only=True)
    ws = wb[sheet]
...
def sap_v3_tfl_rows() -> list[dict[str, str]]:
    doc = Document(ROOT / "02_specifications/sap/TROPIC_SAP_v3.0.docx")
    t = doc.tables[20]
    headers = [c.text.strip() for c in t.rows[0].cells]
    rows = []
    produced = {p.name for p in (ROOT / "05_outputs/tfl" / "output").glob("**/*") if p.is_file()}
...
def source_register_rows() -> list[list[str]]:
    return [
        [
            ...
            "01_source_data/Sanofi Study Protocol Tropic.pdf",
        ...
        [
            "Audit evidence pack",
            "06_qc_evidence/audit/AUDIT_REPORT.md and 06_qc_evidence/audit/findings_register.csv",
        ...
        [
            ...
            "01_source_data/reconstruct_cbzp_guyot.R; 01_source_data/reconstruct_cbzp_arm.R; 01_source_data/guyot_digitised/*",
...
def build_doc() -> Document:
    ...
    add_source(
        doc,
        "Protocol PDF pages 3, 10-12; Lancet PDF pages 1, 4-5, 8; 06_qc_evidence/audit/AUDIT_REPORT.md; official FDA/CDISC/ICH sources listed in References.",
    )
    ...
    add_table(
        ...
        [
            ...
            ["TROPIC protocol", "01_source_data/Sanofi Study Protocol Tropic.pdf"],
            ["TROPIC publication", "01_source_data/reference_literature/de_bono_lancet_2010.pdf"],
        ],
        widths=[2.5, 4.0],
        font_size=7.2,
    )
