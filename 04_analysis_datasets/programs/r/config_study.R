# config_study.R — read from governed config/study_config.yaml
# Version: 3.5.0
# Date: 2026-06-12
# Reference: TROPIC SAP v4.0 controlled draft (EFC6193 / XRP6258)

config_file <- "config/study_config.yaml"
if (!file.exists(config_file)) {
  if (file.exists(file.path("..", "config/study_config.yaml"))) {
    config_file <- file.path("..", "config/study_config.yaml")
  } else {
    stop("config/study_config.yaml not found in . or ..")
  }
}
...
# spec-sourced variable labels (GENERATED: platform/gen_adam_labels.R from the authoritative
# 03_metadata/adam/ADaM_spec.xlsx — audit C-4 inversion). Applied so the R validation-track datasets
# carry the same spec labels as the SAS production track — closes the missing-label ADaM conformance
# findings (06_qc_evidence/conformance/adam_conformance_report.md) symmetrically.
.adam_label_spec <- local({
  p <- Filter(file.exists, c("04_analysis_datasets/programs/r/adam_var_labels.csv", "adam_var_labels.csv"))
