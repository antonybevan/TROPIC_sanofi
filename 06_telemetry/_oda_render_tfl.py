...
def main(argv=None):
    ...
    cbz_xpts = [
        os.path.join(PROJECT_ROOT, "01_raw_source", "cbzp_reconstructed", f"{d}_cbzp.xpt")
        for d in CBZ_DOMS
    ]

    if any(not os.path.exists(p) for p in cbz_xpts):
        rscript = shutil.which("Rscript") or "Rscript"
        print("CbzP XPT bridge files missing -> running 01_raw_source/export_cbzp_xpt.R ...",
              flush=True)
        for p in cbz_xpts:
            try:
                os.remove(p)
                ...
                pass
        try:
            subprocess.run([rscript, os.path.join(PROJECT_ROOT, "01_raw_source",
                                                  "export_cbzp_xpt.R")],
    ...
    if missing:
        sys.exit(f"ERROR: missing CbzP XPT bridge files after export attempt: {missing}.\n"
                 ...
                 f"(run 01_raw_source/reconstruct_cbzp_arm.R).")
    if not programs:
...
""", timeout_s=exec_timeout)
            ...
            with open(os.path.join(PROJECT_ROOT, "02_production_sas", "oda_master_driver.log"),
                      "w") as fh:
                fh.write(r["LOG"])
            ...
            if errs:
                ...
                for e in errs[:20]:
                    print("  ", e, flush=True)
                print("  See 02_production_sas/oda_master_driver.log", flush=True)
                sys.exit(1)
...
""", timeout_s=exec_timeout)
        ...
        if errs2:
            print(f"RESULT: T_tfl_generation had {len(errs2)} ERROR line(s) — "
                  f"see 02_production_sas/oda_tfl.log", flush=True)
            sys.exit(2)

        os.makedirs(os.path.join(PROJECT_ROOT, "04_adam"), exist_ok=True)
        os.makedirs(os.path.join(PROJECT_ROOT, "09_tfl", "output", "figures", "sas"),
                    exist_ok=True)
        if not TFL_ONLY:
        ...
        fig_fail = []
        for fig in FIGURES:
            local = os.path.join(PROJECT_ROOT, "09_tfl", "output", "figures", "sas",
                                 f"{fig}.png")
        ...
        for name in FIGURE_DATA_FILES:
            ok, msg = _download_checked(sas, os.path.join(PROJECT_ROOT, "04_adam", name),
        ...
        print("\nSAS TFL RENDER COMPLETE — all figures downloaded to "
              "09_tfl/output/figures/sas/.", flush=True)
    finally:
