...
def _stage_rows(manifest):
    ...
    for d in manifest.get("datasets", []):
        ...
        stages.append({
            "name": name,
            "script": f"03_validation_r/{d['val']}",
            "runner": "logrx",
...
def build_gate_map(manifest_path, delivery_path, out_dir, report_path):
    ...
    lines = [
        ...
        "",
        "> Maps the actual `study_manifest.yaml` pipeline stages to the professional delivery gates in `delivery_workstreams.yaml`. "
...
def main(argv=None):
    ...
    parser.add_argument("--manifest", default="study_manifest.yaml")
    parser.add_argument("--delivery", default="delivery_workstreams.yaml")
    parser.add_argument("--out-dir", default="06_telemetry/orchestrator_gate_map")
