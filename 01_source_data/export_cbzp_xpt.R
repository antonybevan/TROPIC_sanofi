...
#   - one XPT per domain at 01_source_data/cbzp_reconstructed/<dom>_cbzp.xpt
#   - the single member is named UPCASE(<dom>)_C  (e.g. adtte -> ADTTE_C), which
#     the SAS side references as  set _cz.<UPCASE(dom)>_C;
#
# Run: Rscript 01_source_data/export_cbzp_xpt.R   (from project root)
# ==============================================================================
suppressMessages({
  library(haven)
})
...
for (d in doms) {
  ...
  if (!file.exists(rds)) {
    stop(sprintf("Missing %s -- run 01_source_data/reconstruct_cbzp_arm.R first.", rds))
  }
