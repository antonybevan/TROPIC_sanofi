...
|---|---|---|---|
| Tabulation (SDTM) | SDTMIG | **3.4** (uplifted from 3.1.1 source — see §3) | `03_metadata/define/define_sdtm.xml` |
| Analysis (ADaM) | ADaMIG | 1.3 | `03_metadata/define/define.xml` |
| Occurrence (AE) | OCCDS | 1.0 (+ documented episode-merging extension) | ADRG §2 |
| Metadata | Define-XML | 2.1.0 (+ ARM 1.0) | both define files |
| Analysis results metadata | Analysis Results Standard (ARS) | 1.0 | `05_outputs/ars/tropic_reporting_event.json` |
| Controlled Terminology | CDISC/NCI CT | **2026-03-27** (SDTM + ADaM) | both define files |
| Study definition | USDM (DDF) | 3.0.0 | `03_metadata/usdm/tropic_usdm.json` |
| Transport | SAS Transport v5 (XPT) **and** CDISC Dataset-JSON | v5 + 1.1.0 | `04_analysis_datasets/adam/*.xpt`, `04_analysis_datasets/datasetjson/` |
| Submission packaging | eCTD | ICH 3.2 backbone + STF 2.2 + FDA Regional v3.3 (**all DTD-valid**) | `08_submission_package/ectd/0000/` |
...
1. **SDTMIG version — RESOLVED (2026-06-20).** The PDS source is SDTMIG 3.1.1 (below the
   ...
   pristine source (`01_source_data/real_sdtm/`) is unchanged. See SDRG §5.
...
SDTM + SUPP (`08_submission_package/m5/.../tabulations/sdtm`), ADaM 7 + BIMO clinsite (`04_analysis_datasets/adam/`,
`08_submission_package/m5/.../analysis/adam`), both Define-XML + stylesheets, ADRG/SDRG/BDRG + traceability
matrix, TFLs (`05_outputs/tfl/`), ARS ReportingEvent + ARD (`05_outputs/ars/`), USDM study definition
(`03_metadata/usdm/`), Dataset-JSON (`04_analysis_datasets/datasetjson/`), eCTD backbone + STF (`08_submission_package/ectd/`).

## 5. Standards governance

Pipeline structure and dataset/program/validation wiring are declared once in
`config/study_manifest.yaml`; clinical parameters in `config/study_config.yaml`. Reproducibility is
pinned (`renv.lock`; `00_governance/REPRODUCIBILITY.md`).
