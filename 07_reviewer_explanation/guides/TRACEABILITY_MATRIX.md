...
| **ADTTE** | `A_adtte_generation.sas` | `v_adtte_validation.R` | `IG.ADTTE` | OS; PFS composite includes tumour, PSA, pain progression and death with NACT censoring plus last-evaluable/no-post-baseline censoring; **TTSAE** (was `TTOS`); TTPAIN uses ITT + 5-of-7 diary evaluability; TTPSA and TTUMOR use ITT/randomization origin; TTUMOR uses RECIST-only post-baseline assessments and excludes death milestones | `USUBJID,PARAMCD` (multiset) | ″ |
