...
# 03_metadata/define/define.xml directly (one row per dataset+variable, plus full
...
#       * define.xml is CHECKED AGAINST the spec (03_metadata/define/
#         check_define_conformance.R), not the other way round;
#       * the spec is independently CHECKED AGAINST the produced ADaM data
#         (metatools, 04_analysis_datasets/programs/r/spec_data_checks.R).
...
root <- normalizePath(".")
define_path <- file.path(root, "03_metadata/define", "define.xml")
out_path <- file.path(root, "03_metadata/adam", "ADaM_spec.xlsx")
