...
#
# Data     : ../04_adam/  (production ADaM datasets and reconciled figure data)
...
#              R -e "shiny::runApp('14_shiny', launch.browser = TRUE)"
# ==============================================================================
...
# Configuration
# ------------------------------------------------------------------------------
ADAM_DIR <- normalizePath(file.path("..", "04_adam"), mustWork = FALSE)
...
read_reconciliation <- function() {
  path <- normalizePath(file.path("..", "05_reconciliation",
    ...
    card(
      ...
      card_body(
        p("Independent double programming: each production endpoint is ",
          ...
          tags$code("05_reconciliation/results_reconcile.log"), "."),
