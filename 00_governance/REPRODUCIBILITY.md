...
| `.core_run/` (CORE engine clone, rule cache, converted XPT) · `.core_venv/` (Py 3.12 venv) | Ephemeral CDISC CORE tooling + patient-data XPT; rebuilt by `run_core_conformance.sh`. The CORE **rule pack, reports, and run-record are committed** under `platform/conformance*/`. |
...
```
The runner builds a Py 3.12 venv (`.core_venv`), clones CORE v0.16.0 (`.core_run/engine`; applies a one-line CLI patch so `-s adamig` is accepted), caches library metadata, converts/uplifts SDTM to XPT, then validates **SDTM** twice — the authoritative uplifted layer (`-s sdtmig -v 3.4`) and the pristine-source baseline (`-s sdtmig -v 3.2`) — and **ADaM** (`-s adamig -v 1.3 --local-rules platform/conformance_rules/adam`). Reports → `platform/conformance/`.

### Committed vs. ephemeral, and determinism
- **Committed (inspect without re-running):** the executable rule pack (`platform/conformance_rules/adam/*.yml`), the **redacted** JSON reports, `CORE_RUN_RECORD.md`, and the now-CORE-parseable Define-XML files.
