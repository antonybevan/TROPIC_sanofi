...
**Define-XML conformance.** The analysis metadata (`07_define_xml/define.xml`) **passes full XSD validation** against the official CDISC Define-XML 2.1 + ARM v1.0 schema — run `07_define_xml/validate_xsd.sh` (wraps `xmllint` against the vendored `07_define_xml/schema/` bundle) → *"XSD: VALID."* This covers the schema layer (structure, namespaces, required attributes, enumerations, element ordering) and includes Analysis Results Metadata (ARM) for the OS and PFS analyses. The deeper FDA/CDISC business-rule layer is governed by a [Conformance Validation Plan](file:///Users/apple/Desktop/TROPIC/07_define_xml/CONFORMANCE_VALIDATION_PLAN.md); `validate_define.py` covers the core referential-integrity rules offline. **Business-rule engine status (2026-06-14):** the open-source CDISC CORE engine (v0.16.0) was provisioned and run, but it ships **no executable ADaM rules yet** (SDTM/SEND/TIG/USDM only); therefore **Pinnacle 21** — which has a mature executable ADaM rule pack — is the **required** engine for the ADaM business-rule conformance run (`07_define_xml/P21_RUNBOOK.md`; run record `06_telemetry/p21_conformance_runrecord.md`). The authoritative submission run remains Pinnacle 21 with the FDA Validator Rules matched to the Data Standards Catalog.
