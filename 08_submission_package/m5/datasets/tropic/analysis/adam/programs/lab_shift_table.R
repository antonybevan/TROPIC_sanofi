...
select_lab_worst <- function(lb_data, paramcd_val) {
  lb_data |>
    filter(
      .data$PARAMCD == paramcd_val,
      # SAS/XPORT character missing values are read by haven as either blank
      # strings or NA depending on the producer. Both represent an unflagged
      # record; testing only is.na() silently drops valid post-baseline rows.
      coalesce(as.character(.data$ABLFL), "") != "Y",
      .data$ANL01FL == "Y",
      !is.na(.data$ATOXGR)
    ) |>
...
build_lab_shift_result <- function(lb_data, paramcd_val, safety_n) {
  ...
  if (safety_n > 0L && shift_n == 0L) {
    stop(sprintf(
      "%s shift denominator is zero for a non-empty safety population (N=%d)",
      paramcd_val, safety_n
    ))
  }
  if (shift_n > safety_n) {
    stop(sprintf(
