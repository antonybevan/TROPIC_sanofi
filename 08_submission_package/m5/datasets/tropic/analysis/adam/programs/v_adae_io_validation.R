...
df_ae <- ae |>
  ...
  mutate(
    ...
    CQ02NAM = if_else(
      AEDECOD %in% c("NEUTROPENIA", "FEBRILE NEUTROPENIA", "LEUKOPENIA"),
      "HEMATOLOGIC EVENT",
      ""
    ),
    CQ02CD = if_else(
    ...
    aetrtem_clean = if_else(
      is.na(AETRTEM) | trimws(AETRTEM) == "",
      NA_character_,
      trimws(AETRTEM)
    )
  )
...
# as the final sort key wherever (USUBJID, dates, term) ties occur. This
# mirrors the input record order SAS preserves through its DATA steps WITHOUT
# reading the SAS production dataset, so the QC track remains genuinely
# independent.
df_ae <- df_ae |> mutate(AESEQ = as.numeric(AESEQ))

# Implement Episode Merging and Denominator occurrence flagging
# Sort by usubjid CQ02NAM astdt aendt, with AESEQ as the deterministic
# tie-breaker
df_sorted <- df_ae |>
  arrange(
    USUBJID, CQ02NAM, desc(is.na(astdt)), astdt,
    desc(is.na(aendt)), aendt, AESEQ
  )

# Episode merging using running-max of aendt (matching SAS retain-based logic).
# SAS retains _ciaeedt across rows; when aendt is missing,
# max(retained, .) yields retained.
    ...
    is_new_seq = {
      ...
      for (i in seq_len(n)) {
        if (i == 1 || CQ02NAM[i] == "") {
          ...
          if (is.na(running_end) ||
                astdt[i] > (running_end + EPISODE_GAP_DAYS)) {
            result[i] <- 1.0
            running_end <- aendt[i]
          } else {
            result[i] <- 0.0
...
adae_final <- adae_pre |>
  arrange(
    USUBJID, AEDECOD, desc(is.na(astdt)), astdt,
    desc(is.na(aendt)), aendt, AESEQ
  ) |>
  ...
  select(
    ...
    CQ02NAM, CQ02CD, CQ02SC,
    CIAESEQ = ciaeseq, CIAESDT = ciaesdt,
    CIAEEDT = ciaeedt, CIAEDUR = ciaedur,
...
for (.dv in names(adae)) {
  if (inherits(adae[[.dv]], "Date")) {
    attr(adae[[.dv]], "format.sas") <- "DATE9."
  }
}
write_xpt_v(adae, "04_analysis_datasets/adam/adae_v.xpt", domain = "ADAE")

cat("NOTE: [VALIDATION] Wrote validation ADAE: 04_analysis_datasets/adam/adae_v.xpt\n")
