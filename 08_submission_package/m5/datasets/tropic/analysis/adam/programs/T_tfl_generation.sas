...
/* ==============================================================================
   ...
   Description: SAS production-track statistical figures, rendered natively via ODS
                Graphics. The R / pharmaverse track (05_outputs/tfl/tfl_generation.R) is the
   ...
   Output: &PROJ_ROOT/05_outputs/tfl/output/sas/F-*.png  (300 dpi PNG, Lancet style)
   ============================================================================= */
...
%let CBZDIR = &PROJ_ROOT.&PATH_SEP.01_source_data&PATH_SEP.cbzp_reconstructed;
%let SASFIG = &PROJ_ROOT.&PATH_SEP.05_outputs/tfl&PATH_SEP.output&PATH_SEP.figures&PATH_SEP.sas;

/* Ensure the SAS figure output directory tree exists on the host (no XCMD needed) */
data _null_;
    if fileexist("&PROJ_ROOT.&PATH_SEP.05_outputs/tfl") = 0
        then rc1 = dcreate('05_outputs/tfl', "&PROJ_ROOT.");
    if fileexist("&PROJ_ROOT.&PATH_SEP.05_outputs/tfl&PATH_SEP.output") = 0
        then rc2 = dcreate('output', "&PROJ_ROOT.&PATH_SEP.05_outputs/tfl");
    if fileexist("&PROJ_ROOT.&PATH_SEP.05_outputs/tfl&PATH_SEP.output&PATH_SEP.figures") = 0
        then rc3 = dcreate('figures', "&PROJ_ROOT.&PATH_SEP.05_outputs/tfl&PATH_SEP.output");
    if fileexist("&SASFIG.") = 0
        then rc4 = dcreate('sas', "&PROJ_ROOT.&PATH_SEP.05_outputs/tfl&PATH_SEP.output&PATH_SEP.figures");
run;
...
proc export data=figure_km_stats
    outfile="&PROJ_ROOT.&PATH_SEP.04_analysis_datasets/adam&PATH_SEP.figure_km_stats_prod.csv"
    dbms=csv replace;
run;
proc export data=figure_km_risk
    outfile="&PROJ_ROOT.&PATH_SEP.04_analysis_datasets/adam&PATH_SEP.figure_km_risk_prod.csv"
    dbms=csv replace;
run;
...
/* Export the figure's own forest HRs for numerical SAS<->R reconciliation
   (06_qc_evidence/reconciliation/forest_reconcile.R). Exporting the figure dataset (not an
...
proc export data=forest(keep=subgroup hazardratio waldlower waldupper)
    outfile="&PROJ_ROOT.&PATH_SEP.04_analysis_datasets/adam&PATH_SEP.forest_hr_prod.csv"
    dbms=csv replace;
run;
...
proc export data=_psab(keep=usubjid trt01p rank best respcat)
    outfile="&PROJ_ROOT.&PATH_SEP.04_analysis_datasets/adam&PATH_SEP.figure_waterfall_prod.csv"
    dbms=csv replace;
run;
...
proc export data=_swim30(keep=usubjid trt01p row durm death)
    outfile="&PROJ_ROOT.&PATH_SEP.04_analysis_datasets/adam&PATH_SEP.figure_swimmer_prod.csv"
    dbms=csv replace;
run;
...
proc export data=_er(keep=usubjid trt01p rdi anc loganc)
    outfile="&PROJ_ROOT.&PATH_SEP.04_analysis_datasets/adam&PATH_SEP.figure_er_prod.csv"
    dbms=csv replace;
run;
proc format;
    value loganc
